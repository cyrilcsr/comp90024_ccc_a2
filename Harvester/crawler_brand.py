...
while True:
    ...
    wfile = open("record_b.json", 'a', encoding="utf-8")
    try:
        idfile = open("id_b.txt", 'r')
    ...
    try:
        if id_list[0] == "" and len(id_list) == 1:
            new_file = True
            search_results = api.search(q="(Astrazeneca OR AstraZeneca OR BioNTech OR Pfizer OR "
                                          "Moderna OR Sinovac OR Sputnik) place:%s" % place_id, count=100,
                                        tweet_mode="extended")
        else:
